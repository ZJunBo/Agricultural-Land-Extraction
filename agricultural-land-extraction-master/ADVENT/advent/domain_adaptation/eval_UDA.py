...
def eval_best(cfg, models,
    ...
    for i_iter in range(start_iter, max_iter + 1, step):
        ...
        if i_iter not in all_res.keys():
            ...
            for index in tqdm(range(len(test_loader))):
                ...
                if not fixed_test_size:
                    interp = nn.Upsample(size=(label.shape[1], label.shape[2]), mode='bilinear', align_corners=True)
                with torch.no_grad():
                    # print(models[0](image.cuda(device)).shape)
                ...
                if TP != 0 and TP_FN != 0 and TP_FP != 0:
                    COM = float(TP) / float(TP_FN)
                    COR = float(TP) / float(TP_FP)
                    Q = float(TP) / float(TP_FN + TP_FP - TP)
        ...
        print("begin")
        COM_avg = np.mean(COM_all)
        COR_avg = np.mean(COR_all)
        Q_avg = np.mean(Q_all)
        F1_avg = np.mean(F1_all)
