...
cfg.EXP_ROOT_SNAPSHOT = osp.join(cfg.EXP_ROOT, 'GID_LoveDA')
# CUDA
cfg.GPU_ID =0
# TRAIN CONFIGS
cfg.TRAIN = EasyDict()
...
cfg.TRAIN.MODEL = 'Transformer'
cfg.TRAIN.MULTI_LEVEL = True
cfg.TRAIN.RESTORE_FROM = ''
cfg.TRAIN.IMG_MEAN = np.array((0, 0, 0), dtype=np.float32)
# deepglobe
cfg.TRAIN.IMG_MEAN = np.array((40.990373, 38.057142, 27.83476), dtype=np.float32)

cfg.TRAIN.LEARNING_RATE = 4e-6  # 分割
cfg.TRAIN.MOMENTUM = 0.9
cfg.TRAIN.WEIGHT_DECAY = 0.0005
cfg.TRAIN.POWER = 0.9
cfg.TRAIN.LAMBDA_SEG_MAIN = 1.0
cfg.TRAIN.LAMBDA_SEG_AUX = 0.4  # weight of conv4 prediction. Used in multi-level setting.
# Domain adaptation
cfg.TRAIN.DA_METHOD = 'AdvEnt'
# Adversarial training params
cfg.TRAIN.LEARNING_RATE_D = 4e-7   # 判别器
cfg.TRAIN.LAMBDA_ADV_MAIN = 0.0001  # 对抗
...
# model
cfg.TEST.MODEL = ('Transformer',)
cfg.TEST.MODEL_WEIGHT = (1.0,)
cfg.TEST.MULTI_LEVEL = (True,)
cfg.TEST.IMG_MEAN = np.array((0, 0, 0), dtype=np.float32)
# cfg.TEST.IMG_MEAN = np.array((41.5194, 38.0485, 27.6734), dtype=np.float32)
cfg.TEST.RESTORE_FROM = ('',)
cfg.TEST.SNAPSHOT_DIR = ('',)  # used in 'best' mode
cfg.TEST.SNAPSHOT_STEP = 10 # used in 'best' mode
cfg.TEST.SNAPSHOT_MAXITER = 100  # used in 'best' mode
# Test sets
