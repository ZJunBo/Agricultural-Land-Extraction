...
class MultiScaleFeaturFusion(nn.Module):
    def __init__(self, in_dim=[96, 192, 384, 768],ppm_pool_scale=[1, 2, 3, 6], ppm_dim=512, fpn_out_dim=256):
        super(MultiScaleFeaturFusion, self).__init__()
        # PPM ----
        dim = in_dim[-1]
        ppm_pooling = []
        ppm_conv = []
    ...
    def forward(self, feature):
        ...
        ppm_out = torch.cat(ppm_out, 1)#[4,2816,16,16]
        down = self.ppm_out(ppm_out) #[4,256,16,16]
        ...
        fpn_out.reverse()  # [4,256,128,128]  [4,256,64,64] [4,256,32,32] [4,256,16,16]


        fusion_shape = fpn_out[0].shape[2:]
        fusion = [fpn_out[0]]  # [4,256,128,128]
        for i in range(1, len(fpn_out)):
            fusion.append(
                F.interpolate(fpn_out[i], fusion_shape, mode='bilinear', align_corners=False)
            )
        # fusion [4,256,128,128] [4,256,128,128] [4,256,128,128] [4,256,128,128]
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.decoder = MultiScaleFeaturFusion(in_dim=[embed_dim,2*embed_dim,4*embed_dim,8*embed_dim], ppm_pool_scale=[1,2,3,6], ppm_dim=512,fpn_out_dim=256)

        #Multi-scale feature fusion
        self.cls_pred = nn.Conv2d(256,2,1)
    ...
    def forward(self, x):
        ...
        for i in range(self.num_layers): # num_layers=4
            ...
            if i in self.out_indices:  # out_indices=(0, 1, 2, 3),
                norm_layer = getattr(self, f'norm{i}')  #getattr获取对象属性值
                x_out = norm_layer(x_out)
                out = x_out.view(-1, H, W, self.num_features[i]).permute(0, 3, 1, 2).contiguous()
                outs.append(out)

        result = self.decoder(outs)
        result = self.cls_pred(result)
        return result
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        """
        B_, N, C = x.shape
        qkv = self.qkv(x).reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)   # torch.Size([3, 36, 3, 49, 32])
        q, k, v = qkv[0], qkv[1], qkv[2]  # make torchscript happy (cannot use tensor as tuple)  # 都是 torch.Size([36, 3, 49, 32])
        q = q * self.scale
        attn = (q @ k.transpose(-2, -1))  # torch.Size([36, 3, 49, 49])
